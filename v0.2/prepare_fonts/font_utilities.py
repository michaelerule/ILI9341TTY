...
def bitpack_row(r):
  '''
  Pack a row of binary bit data into an array of uint8_t
  '''
  NR = len(r)
  NB = int(ceil(NR/8))
  Nb = NB*8
  if Nb>NR:
    r = concatenate([r,zeros(Nb-NR)])
  r = r.reshape(NB,8)
  return r @ 2**arange(8)
  

################################################################################
def pack_array(data,name):
  '''
  Generate source code to pack array data in C static const array
  '''
  data = array(data).ravel()
  t = None
  if   all((data>=     0)&(data<  256)): t='byte'
  elif all((data>=  -128)&(data<  128)): t='int8_t'
  elif all((data>=     0)&(data<65536)): t='unsigned int'
  elif all((data>=-32768)&(data<32768)): t='int'
  if t is None: raise ValueError('Could not guess datatype');
  s = '\n'
  l = 'static const %s %s[] PROGMEM = {'%(t,name)
...
################################################################################
def get_character_edge_padding(character_pixels):
  CH, CW       = character_pixels.shape
  full_cols    = np.any(character_pixels>0, axis=0)
  full_rows    = np.any(character_pixels>0, axis=1)
  empty_left   = 0
  empty_right  = CW-1
  empty_top    = 0
  empty_bottom = CH-1
  while empty_left   < CW and not full_cols[empty_left  ]: empty_left   +=1
  while empty_top    < CH and not full_rows[empty_top   ]: empty_top    +=1
  while empty_right  >= 0 and not full_cols[empty_right ]: empty_right  -=1
  while empty_bottom >= 0 and not full_rows[empty_bottom]: empty_bottom -=1
  empty_bottom = CH-1-empty_bottom
  empty_right  = CW-1-empty_right
  return empty_left, empty_right, empty_bottom, empty_top

################################################################################
def generate_codepoint_name_abbreviation(c, aliases, abbreviation_map):
    '''
    Generates an abbreviated name for a given unicode point. The unicode name is
    heuristically abbreviated to a 2-5 character literal that can be used to 
    refer to it in the C source code.
